package com.faendir.intellij.gradleVersionCatalogs.groovy
...
//            val returnType = references.map { it.resolve() }.firstIsInstanceOrNull<PsiMethod>()?.returnType ?: return Accessor(element, "koin-android", VCElementType.LIBRARY)
            val segments by lazy { element.text.replace(Regex("\\s+"), "").split(".").drop(1)
                ...
                .map { it.removePrefix("get").removeSuffix("()") }.filterNot(String::isEmpty)
            }
            val type = when {
...
//    GroovyPsiManager.getInstance(context.project).createTypeByFQClassName(fqn, context.resolveScope)
